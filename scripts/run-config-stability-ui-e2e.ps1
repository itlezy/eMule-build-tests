...
$uiReportRoot = Join-Path $reportRoot 'config-stability-ui-e2e'
...
$latestReportDir = Join-Path $reportRoot 'config-stability-ui-e2e-latest'
$sourceArtifactsRoot = if ([string]::IsNullOrWhiteSpace($ArtifactsRoot)) {
    Join-Path ([System.IO.Path]::GetTempPath()) ('emule-config-stability-ui-e2e-' + [Guid]::NewGuid().ToString('N'))
} else {
...
$pythonScriptPath = Join-Path $PSScriptRoot 'config-stability-ui-e2e.py'
$runStatus = 'failed'
$errorMessage = ''
...
if ($runStatus -eq 'passed') {
    Write-Host "Config-stability UI E2E passed. Report directory: $runReportDir"
    return
}

Write-Host "Config-stability UI E2E failed. Report directory: $runReportDir" -ForegroundColor Red
throw $(if ([string]::IsNullOrWhiteSpace($errorMessage)) { 'Config-stability UI E2E failed.' } else { $errorMessage })
