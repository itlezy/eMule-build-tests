...
TEST_CASE("Part-status ownership seam preserves the legacy NULL-for-empty raw view contract")
{
	...
	const std::vector<uint8> &rConstStatus = status;
	const uint8 *pConstView = PartStatusOwnershipSeams::GetRawStatusView(rConstStatus);
	REQUIRE(pConstView != NULL);
	CHECK(pConstView[1] == static_cast<uint8>(1u));
}
...
TEST_CASE("Part-status ownership seam builds bounded pending overlays and display buffers")
{
	...
	REQUIRE(PartStatusOwnershipSeams::TryBuildPendingPartOverlay(0u, overlay));
	CHECK(overlay.empty());
	...
	std::vector<uint8> emptyStatus;
	std::vector<TCHAR> emptyDisplay;
	REQUIRE(PartStatusOwnershipSeams::TryBuildPartStatusDisplay(emptyStatus, emptyDisplay));
	CHECK_EQ(emptyDisplay.size(), static_cast<size_t>(1u));
	CHECK(emptyDisplay[0] == _T('\0'));
