...
TEST_CASE("Kad base publish and search limits keep the expected relative ceilings")
{
	CHECK(KADEMLIATOTALSTOREKEY > 0);
	CHECK(KADEMLIATOTALSTORESRC > 0);
	CHECK(KADEMLIATOTALSTORENOTES > 0);
	CHECK(KADEMLIAPUBLISHTIME > 0);
	CHECK(SEARCHFINDSOURCE_LIFETIME >= SEARCHNODE_LIFETIME);
}
