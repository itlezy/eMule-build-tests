...
[CmdletBinding()]
param(
    [string]$TestRepoRoot = (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)),
...
$ActiveScriptsRoot = Split-Path -Parent $PSScriptRoot

Write-Warning 'scripts\obsolete\run-live-diff.ps1 is obsolete; use scripts\run_live_diff.py for new work.'

. (Join-Path $ActiveScriptsRoot 'resolve-workspace-layout.ps1')
